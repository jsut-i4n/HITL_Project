...
from .current_time import get_current_date
...
def load_all_scene_configs():
    ...
    config_path = os.path.join(current_dir, 'scene_config.json')  # 搜索目录下的所有json文件
    current_config = load_scene_templates(config_path)
...
def get_slot_update_message(scene_name, dynamic_example, slot_template, user_input):
    message = scene_prompts.slot_update.format(scene_name, get_current_date(), json.dumps(get_slot_update_json(slot_template), ensure_ascii=False), dynamic_example, user_input)
    return message
